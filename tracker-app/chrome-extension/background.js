/**
 * JobTracker LinkedIn Connect — Background Service Worker v4.0.0
 *
 * v4.0.0 — Remove auto-navigation for ATS jobs:
 * - REMOVED: applyAtsDirectly action handler (extension no longer opens tabs for ATS)
 * - REMOVED: jobBoardClickApplyScript function
 * - REMOVED: JOB_BOARD_PATTERNS array and detectJobBoard function
 * - REMOVED: Cloudflare challenge detection code (clickCfCheckbox, CF title polling)
 * - ATS jobs are now handled by the user navigating manually; the extension auto-fills
 *   when it detects an ATS page via the onUpdated listener + pendingExternalApply.
 * - LinkedIn Easy Apply flow is unchanged (applyViaExtension still opens tabs).
 *
 * v3.1.0 — Job Board New-Tab Redirect Fix (removed in v4.0.0)
 * v3.0.0 — Job Board Redirect Flow (removed in v4.0.0)
 * v2.9.0 — pending_external handling for linkedin-apply.js
 * v2.8.0 — Auto re-inject content scripts after extension reload (onInstalled)
 * v2.7.0 — Separate focused window for LinkedIn SPA rendering
 * v2.6.0 — Snapshot tab IDs, close stale ATS tabs after result
 * v2.5.0 — Background tabs, event-driven tab load, mid-flight re-injection
 */
